...
export async function POST(request) {
  try { const { name, email, password } = await request.json();
    ...
    const token = newSessionToken(); const user = await User.create({ name: name.trim(), email: email.toLowerCase(), password: await bcrypt.hash(password, 12), sessionToken: token }); await setSession(token);
